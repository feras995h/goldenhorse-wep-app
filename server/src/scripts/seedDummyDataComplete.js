...
// Helper function to create or find customer (using raw SQL)
const createOrFindCustomer = async (customerData) => {
  try {
    const [existingCustomers] = await sequelize.query(
      ...
      {
        replacements: { code: customerData.code },
    ...
    const [newCustomer] = await sequelize.query(
      `INSERT INTO customers (id, code, name, "nameEn", "contactPerson", phone, email, address, "taxNumber", "creditLimit", "isActive", balance, "createdAt", "updatedAt") 
       VALUES (:id, :code, :name, :nameEn, :contactPerson, :phone, :email, :address, :taxNumber, :creditLimit, :isActive, :balance, :createdAt, :updatedAt) 
       RETURNING *`,
      {
        replacements: {
          id: uuidv4(),
    ...
    if (error.name === 'SequelizeUniqueConstraintError') {
      console.log(`✅ Customer already exists (caught error): ${customerData.code} - ${customerData.name}`);
...
// Helper function to create or find employee (using raw SQL)
const createOrFindEmployee = async (employeeData) => {
  try {
    const [existingEmployees] = await sequelize.query(
      'SELECT * FROM employees WHERE "employeeNumber" = :employeeNumber',
      {
        replacements: { employeeNumber: employeeData.employeeNumber },
        type: sequelize.QueryTypes.SELECT
      }
    );
    ...
    const [newEmployee] = await sequelize.query(
      `INSERT INTO employees (id, "employeeNumber", name, "nameEn", position, department, "hireDate", salary, phone, email, address, "isActive", "createdAt", "updatedAt") 
       VALUES (:id, :employeeNumber, :name, :nameEn, :position, :department, :hireDate, :salary, :phone, :email, :address, :isActive, :createdAt, :updatedAt) 
       RETURNING *`,
      {
        replacements: {
          id: uuidv4(),
    ...
    if (error.name === 'SequelizeUniqueConstraintError') {
      console.log(`✅ Employee already exists (caught error): ${employeeData.employeeNumber} - ${employeeData.name}`);
      // Try to get the existing employee
      const [existingEmployees] = await sequelize.query(
        'SELECT * FROM employees WHERE "employeeNumber" = :employeeNumber',
        {
          replacements: { employeeNumber: employeeData.employeeNumber },
...
const seedDummyDataComplete = async () => {
  try {
    ...
    // Get all customers from database to ensure we have their IDs
    const allCustomers = await sequelize.query(
      'SELECT * FROM customers WHERE code IN (:codes)',
      {
        replacements: { codes: customers.map(c => c.code) },
        type: sequelize.QueryTypes.SELECT
      }
    );
    ...
    // Get all employees from database to ensure we have their IDs
    const allEmployees = await sequelize.query(
      'SELECT * FROM employees WHERE "employeeNumber" IN (:employeeNumbers)',
      {
        replacements: { employeeNumbers: employees.map(e => e.employeeNumber) },
        type: sequelize.QueryTypes.SELECT
      }
    );
    ...
    console.log('🎉 Complete dummy data seeding completed successfully!');
    console.log('📊 Summary:');
    console.log('- 2 additional users created');
    console.log('- 5 additional accounts created');
    ...
    console.log('- 5 settings created');
...
seedDummyDataComplete()
  .then(() => {
    console.log('✅ Complete dummy data seeding completed');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ Complete dummy data seeding failed:', error);
    process.exit(1);
  });
