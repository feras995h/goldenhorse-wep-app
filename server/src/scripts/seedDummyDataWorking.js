...
console.log('🌱 Starting to seed working dummy data...');
...
// Helper function to create or find customer (using raw SQL to avoid Sequelize caching issues)
...
const seedDummyDataWorking = async () => {
  try {
    ...
    // Create customers (using raw SQL to avoid Sequelize caching issues)
    ...
    console.log('🎉 Working dummy data seeding completed successfully!');
    console.log('📊 Summary:');
...
seedDummyDataWorking()
  .then(() => {
    console.log('✅ Working dummy data seeding completed');
    process.exit(0);
  })
  .catch((error) => {
    console.error('❌ Working dummy data seeding failed:', error);
    process.exit(1);
  });
