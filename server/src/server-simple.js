...
app.get('/health', async (req, res) => {
  res.json({
    ...
    message: 'Running in simple mode without Redis'
  });
});
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`📊 Cache service: Not available (Redis not configured)`);
  console.log(`🔄 Realtime service: Not available (Redis not configured)`);
  console.log(`🌐 CORS origin: ${process.env.CORS_ORIGIN || process.env.CLIENT_URL || 'http://localhost:3000'}`);
  console.log(`✅ Running in simple mode - no Redis required`);
});
