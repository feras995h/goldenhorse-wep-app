...
const sequelize = new Sequelize(databaseUrl, {
  ...
  dialectOptions: {
    // تعطيل SSL إذا كان الخادم لا يدعمه
    ssl: false
  },
  pool: {
    max: 5,
    ...
    for (const accountData of chartOfAccounts) {
      try {
        ...
        if (existingResult.length === 0) {
          // البحث عن parentId إذا كان parentCode موجود
          let parentId = null;
          if (accountData.parentCode) {
            const [parentResult] = await sequelize.query(
              'SELECT id FROM accounts WHERE code = :parentCode',
              { replacements: { parentCode: accountData.parentCode } }
            );
          ...
          await sequelize.query(`
            INSERT INTO accounts (
              ...
              "isGroup", "parentId", balance, "isActive", "accountType",
              nature, currency, level, "createdAt", "updatedAt"
            )
            VALUES (
              gen_random_uuid(), :code, :name, :nameEn, :type, :rootType, :reportType,
              :isGroup, :parentId, :balance, true, :accountType,
              :nature, 'LYD', :level, NOW(), NOW()
            )
          `, {
            replacements: {
              ...
              accountType: parentId ? 'sub' : 'main',
              nature: ['asset', 'expense'].includes(accountData.type) ? 'debit' : 'credit',
              level: parentId ? 2 : 1
            }
          });
