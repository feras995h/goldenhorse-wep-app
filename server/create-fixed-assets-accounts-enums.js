// Create basic fixed assets accounts with correct ENUM values
import { Sequelize } from 'sequelize';

// Direct database connection using the URL you provided
const DATABASE_URL = 'postgres://postgres:XIclgABy2kg3ZZ2Nyh7GOYexxcm206RTNsSAJavhbF4ukgMfDiNqXSOhy8SIALUP@72.60.92.146:5432/postgres';

async function createFixedAssetsAccountsWithEnums() {
  try {
    console.log('🔧 Creating basic fixed assets accounts with correct ENUM values...');
    ...
    // Create fixed assets accounts with correct ENUM values
    const accounts = [
      {
        code: '112',
        name: 'الأصول الثابتة',
    ...
    for (const account of accounts) {
      try {
        ...
        if (existing.length > 0) {
          ...
          // Create the account with correct ENUM values
...
createFixedAssetsAccountsWithEnums();