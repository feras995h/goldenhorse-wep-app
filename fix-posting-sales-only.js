...
async function fixPostingSalesOnly() {
  ...
  try {
    ...
    console.log('\n🔧 إصلاح دالة post_document للمبيعات فقط...\n');

    // إعادة إنشاء دالة post_document مبسطة للمبيعات
    await client.query(`
    ...
    console.log('✅ تم إصلاح دالة post_document للمبيعات');

    // إعادة إنشاء دالة reverse_document مبسطة للمبيعات
    await client.query(`
      ...
      BEGIN
        -- التحقق من صحة نوع المستند
        IF p_document_type NOT IN ('sales_invoice') THEN
          RAISE EXCEPTION 'نوع المستند غير مدعوم: %', p_document_type;
        END IF;
    ...
    console.log('✅ تم إصلاح دالة reverse_document للمبيعات');

    console.log('\n🎉 تم إصلاح جميع دوال الترحيل للمبيعات بنجاح!');
...
fixPostingSalesOnly().catch(console.error);
