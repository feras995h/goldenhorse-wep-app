...
/**
 * إصلاح شامل ونهائي لمشاكل لوحة المبيعات
 * Complete Final Sales Dashboard Fix Script
 */

console.log('🔧 بدء الإصلاح الشامل والنهائي لمشاكل لوحة المبيعات...\n');
...
async function completeFinalFix() {
  try {
    ...
    // الحصول على معرف مستخدم admin للاستخدام في createdBy
    ...
    if (adminUser.length > 0) {
      ...
      console.log('⚠️ لم يتم العثور على مستخدم admin، سيتم إنشاء واحد...');
      const newAdminResult = await sequelize.query(`
        INSERT INTO users (id, username, password, name, role, "isActive", "createdAt", "updatedAt")
        VALUES (gen_random_uuid(), 'admin', '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'مدير النظام', 'admin', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        RETURNING id
      `, { type: sequelize.QueryTypes.INSERT });
      
      if (newAdminResult && newAdminResult[0] && newAdminResult[0][0]) {
        adminUserId = newAdminResult[0][0].id;
    ...
    console.error('❌ خطأ في الإصلاح الشامل والنهائي للوحة المبيعات:', error.message);
