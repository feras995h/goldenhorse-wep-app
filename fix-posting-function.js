...
async function fixPostingFunction() {
  ...
  try {
    ...
    console.log('\n🔧 إصلاح دالة post_document...\n');

    // إعادة إنشاء دالة post_document مع إصلاح الأخطاء
    await client.query(`
      ...
      BEGIN
        ...
        ELSIF p_document_type = 'receipt' THEN
          ...
          IF NOT FOUND THEN
            RAISE EXCEPTION 'الإيصال غير موجود';
          END IF;
          
          IF v_doc_record.posted_status = 'posted' THEN
            RAISE EXCEPTION 'الإيصال مرحل مسبقاً';
          END IF;
          
          v_amount := v_doc_record.amount;
          v_description := 'إيصال قبض رقم ' || v_doc_record.receipt_no;
        END IF;
        ...
        INSERT INTO gl_journals (
          ...
          p_document_id,
          CASE 
            WHEN p_document_type = 'sales_invoice' THEN v_doc_record."invoiceNumber"
            WHEN p_document_type = 'receipt' THEN v_doc_record.receipt_no
          END,
          'posted',
        ...
        ELSIF p_document_type = 'receipt' THEN
          ...
          INSERT INTO gl_entries (
            ...
            v_journal_id, v_cash_account_id, v_amount, 0,
            'مدين الصندوق - ' || v_description, v_line_number
          );
          v_total_debit := v_total_debit + v_amount;
          v_line_number := v_line_number + 1;

          -- دائن: العملاء والمدينون
          INSERT INTO gl_entries (
            journal_id, account_id, debit_amount, credit_amount, 
            description, line_number
          ) VALUES (
            v_journal_id, v_customer_account_id, 0, v_amount,
            'دائن العملاء - ' || v_description, v_line_number
          );
          v_total_credit := v_total_credit + v_amount;
        END IF;
    ...
    console.log('✅ تم إصلاح دالة post_document');

    // إعادة إنشاء دالة reverse_document مع إصلاح الأخطاء
    await client.query(`
      ...
      BEGIN
        ...
        ELSIF p_document_type = 'receipt' THEN
          ...
          IF v_doc_record.posted_status != 'posted' THEN
            RAISE EXCEPTION 'الإيصال غير مرحل';
          END IF;
          
          v_description := 'عكس إيصال قبض رقم ' || v_doc_record.receipt_no;
        END IF;
        ...
        UPDATE gl_journals 
        SET 
          total_debit = (SELECT COALESCE(SUM(debit_amount), 0) FROM gl_entries WHERE journal_id = v_reversal_journal_id),
          total_credit = (SELECT COALESCE(SUM(credit_amount), 0) FROM gl_entries WHERE journal_id = v_reversal_journal_id)
    ...
    console.log('✅ تم إصلاح دالة reverse_document');

    console.log('\n🎉 تم إصلاح جميع دوال الترحيل بنجاح!');
...
fixPostingFunction().catch(console.error);
