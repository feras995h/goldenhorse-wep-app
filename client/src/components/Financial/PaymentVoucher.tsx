...
import { X, Plus, Trash2, Calculator, CreditCard, Search } from 'lucide-react';
...
const PaymentVoucher: React.FC<PaymentVoucherProps> = ({
  ...
  const validateForm = () => {
    ...
    if (totalAllocated > paymentAmount) {
      newErrors.allocations = 'إجمالي المبالغ المخصصة أكبر من مبلغ الدفعة';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await financialAPI.createPaymentVoucher(submitData);
      ...
      setErrors({ submit: error.response?.data?.message || 'حدث خطأ أثناء إنشاء إيصال الصرف' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-bold text-gray-900">إيصال صرف جديد</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6">
                ...
                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                  {accounts.map((account) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-500">الرصيد: {(isNaN(account.balance) || !isFinite(account.balance) ? 0 : account.balance).toLocaleString('ar-LY')} {account.currency || 'LYD'}</div>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="flex justify-end space-x-3 space-x-reverse">
            ...
            >
              {loading ? 'جاري الحفظ...' : 'حفظ إيصال الصرف'}
            </button>
          </div>
        </form>
      </div>
