  ...
  if (printWindow) {
    printWindow.document.write(`
      ...
      <body>
        ...
        <div class="summary">
          <div>الرصيد الافتتاحي: ${new Intl.NumberFormat('ar-LY').format(data.openingBalance || 0)} ${data.currency || 'LYD'}</div>
          <div>إجمالي المدين: ${new Intl.NumberFormat('ar-LY').format(data.totalDebit || 0)} ${data.currency || 'LYD'}</div>
          <div>إجمالي الدائن: ${new Intl.NumberFormat('ar-LY').format(data.totalCredit || 0)} ${data.currency || 'LYD'}</div>
          <div>الرصيد الختامي: ${new Intl.NumberFormat('ar-LY').format(data.closingBalance || 0)} ${data.currency || 'LYD'}</div>
        </div>
