...
/**
 * اختبار النظام المالي المُصلح
 * Test Fixed Financial System
 */

console.log('🧪 بدء اختبار النظام المالي المُصلح...\n');
...
async function testFinancialSystemFixed() {
  try {
    ...
    const receiptData = {
      ...
      amount: 750.0,
      remarks: 'اختبار إنشاء إيصال قبض',
      createdBy: userId,
      completedBy: userId
    };
    ...
    const createPaymentQuery = `
      INSERT INTO payments (
        id, "paymentNo", "accountId", "partyType", "partyId", "customerId", "voucherType",
        "paymentDate", amount, "paymentMethod", "referenceNo", notes,
    ...
    recentPayments.forEach((payment, index) => {
      console.log(`   ${index + 1}. ${payment.paymentNo} - ${payment.amount} د.ل - ${payment.account_name || 'غير محدد'}`);
    });
    ...
    console.log('\n🎉 انتهاء اختبار النظام المالي المُصلح');
    console.log('\n📋 الملخص:');
    ...
    console.log('  ✅ تم حذف البيانات التجريبية');
    console.log('\n🚀 النظام المالي يعمل بكفاءة 100%!');
    
  } catch (error) {
    console.error('❌ خطأ في اختبار النظام المالي:', error.message);
